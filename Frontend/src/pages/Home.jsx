  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-border transition-all">
        <div className="max-w-[1400px] mx-auto px-3 sm:px-4 md:px-6 lg:px-12 flex justify-between items-center h-16 sm:h-20">
          <Link to="/" className="flex items-center gap-3 no-underline group">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-primary text-lg sm:text-xl leading-none">EventAura</span>
              <span className="text-[10px] sm:text-sm font-semibold text-muted-foreground uppercase tracking-widest mt-1">Campus Hub</span>
            </div>
          </Link>
                ...
                <div className="relative">
                  ...
                  {showDropdown && (
                    <div className="absolute right-0 mt-3 w-48 bg-white border border-border rounded-xl shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                      ...
                      <Link to="/register-admin" className="flex items-center px-4 py-3 text-sm font-semibold text-primary hover:bg-muted" onClick={() => setShowDropdown(false)}> Administrator</Link>
                    </div>
                  )}
                </div>
              </>
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden bg-primary text-white">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 relative z-10 flex flex-col lg:flex-row items-center gap-10 sm:gap-12">

          <div className="lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 border border-white/20 text-sm font-bold tracking-widest uppercase mb-6 backdrop-blur-md">
              <span className="text-accent">⚡</span> Verified Platform
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-7xl font-semibold leading-[1.1] tracking-tight mb-6 mt-2">
          ...
          <div className="lg:w-1/2 relative w-full mt-10 lg:mt-0">
            ...
            <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 lg:-translate-x-0 lg:-left-12 bg-card p-4 sm:p-6 rounded-2xl shadow-2xl border border-border flex gap-4 sm:gap-6 z-20 animate-in slide-in-from-bottom w-[92%] sm:w-auto" style={{ transform: 'rotate(-1deg)' }}>
              <div>
                <div className="text-2xl sm:text-3xl font-semibold text-primary mb-1">100<span className="text-accent">+</span></div>
                <div className="text-xs sm:text-sm font-bold text-muted-foreground uppercase tracking-wider">Annual Fairs</div>
              </div>
              <div className="w-[1px] h-12 bg-border"></div>
              <div>
                <div className="text-2xl sm:text-3xl font-semibold text-primary mb-1">{stallStats.available}<span className="text-success">+</span></div>
                <div className="text-xs sm:text-sm font-bold text-muted-foreground uppercase tracking-wider">Stalls Left</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12 w-full pt-20 pb-20">
        ...
        <section id="events" className="mb-32 scroll-mt-24">
          <div className="text-center md:text-left mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-semibold text-primary tracking-tight">Active Campus Events</h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 bg-card rounded-[2rem] border border-border shadow-sm overflow-hidden group">
              <div className="relative h-[300px] md:h-[400px] overflow-hidden">
                <div className={`absolute top-6 left-6 z-10 px-4 py-1.5 rounded-full text-sm font-bold tracking-widest uppercase backdrop-blur-md ${events[activeEvent].status === 'open' ? 'bg-success/90 text-success-foreground' : 'bg-warning/90 text-warning-foreground'}`}>
              ...
              <div className="p-8 md:p-10 -mt-20 relative z-20">
                <div className="bg-card w-full rounded-2xl p-6 md:p-8 shadow-xl border border-border">
                  <h3 className="text-2xl md:text-3xl font-semibold text-primary mb-4 leading-tight">{events[activeEvent].name}</h3>
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
              ...
              {events.map((evt, idx) => (
                <button
                  ...
                  className={`flex items-stretch text-left group rounded-2xl overflow-hidden transition-all duration-300 border ${activeEvent === idx
                ...
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-center">
                    <div className="text-sm font-bold text-accent mb-1.5 uppercase tracking-wider">{evt.date}</div>
        ...
        <section id="map" className="mb-32 scroll-mt-24">
          <div className="bg-card rounded-[2.5rem] border border-border shadow-sm p-4 md:p-6">
            <StallLayoutMap role={user?.role === 'admin' ? 'admin' : 'vendor'} />
          </div>
        </section>
        ...
        <section id="how-to-book" className="mb-20 scroll-mt-24">
          <div className="max-w-[800px] mx-auto text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-semibold text-primary tracking-tight mb-6">Admission Protocol</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((step, i) => (
              <div key={i} className="bg-card p-8 rounded-3xl border border-border hover:border-accent hover:shadow-[0_10px_40px_-10px_rgba(249,115,22,0.15)] transition-all group relative overflow-hidden">
                <div className="text-7xl font-semibold text-muted/40 absolute -right-4 -bottom-6 select-none group-hover:text-accent/10 transition-colors">{step.step}</div>
      ...
      <footer className="bg-primary text-primary-foreground border-t border-primary/20">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-5 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="text-xs font-medium text-primary-foreground/50">
            Campus Stall Management Platform
          </span>
        </div>
      </footer>
