    ...
    <div className="max-w-7xl mx-auto py-8">
      <div className="mb-8">
        <div>
          ...
          <h1 className="text-4xl font-black text-primary tracking-tight">Events Gallery</h1>
      ...
      <div className="mb-8 flex flex-wrap gap-4">
        {['All', 'Food', 'Sponsor', 'Game', 'Shop'].map(type => (
          <button 
            key={type}
          ...
          >
            {type === 'Shop' ? 'Retail' : type === 'All' ? 'All Stalls' : type}
          </button>
        ))}
      </div>
          ...
          <div key={stall._id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-200/40 flex flex-col hover:-translate-y-1 transition-transform duration-300 hover:shadow-2xl hover:shadow-slate-200/50">
            <div className="mb-4 pb-4 border-b border-slate-100">
              <div className="flex justify-between items-start gap-2">
                <div>
                  <div className="font-black text-xl text-primary tracking-tight leading-tight">{stall.stallName}</div>
                  <div className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">{stall.eventName}</div>
                </div>
                <span className="px-2.5 py-1 rounded-lg bg-green-100 text-green-700 text-[0.65rem] font-black uppercase tracking-wider shrink-0 shadow-sm border border-green-200">Available</span>
              </div>
            </div>
            ...
            <div className="flex-1 bg-slate-50/50 rounded-2xl p-4 mb-6 border border-slate-50">
              <div className="grid grid-cols-2 gap-y-4 gap-x-2 text-sm">
                <div className="flex flex-col">
                  <span className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Type</span>
                  <span className="font-bold text-slate-700">{stall.stallType}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Zone</span>
                  <span className="font-bold text-slate-700">{stall.locationZone}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Size Ratio</span>
                  <span className="font-bold text-slate-700">{stall.size}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Cost (LKR)</span>
                  <span className="font-black text-accent">{stall.price.toLocaleString()}</span>
                </div>
              </div>
            </div>
