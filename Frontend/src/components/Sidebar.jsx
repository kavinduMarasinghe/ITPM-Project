...
const Sidebar = ({ role }) => {
  ...
  const NavItem = ({ to, end, icon, label }) => (
    ...
    >
      <SVGIcon d={icon} />
      <span className={`transition-opacity duration-300 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100 block'}`}>
        {label}
      </span>
    </NavLink>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-6 py-8">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-lg shadow-primary/20">
          e
        </div>
        {!isCollapsed && (
          <div className="flex flex-col overflow-hidden">
            <span className="text-white font-black text-xl tracking-tight leading-none">EventAura</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-8 scrollbar-hide">
        ...
        {currentRole === 'admin' && (
          <div>
            {!isCollapsed && <div className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-4 mb-3">Main Menu</div>}
            {isCollapsed && <div className="h-px bg-slate-800 w-full mb-3"></div>}
            <nav className="space-y-1.5">
              <NavItem to="/admin/stalls" end icon={Icons.dashboard} label="Dashboard" />
        ...
        {currentRole === 'admin' && (
          <div>
            {!isCollapsed && <div className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-4 mb-3">General</div>}
            {isCollapsed && <div className="h-px bg-slate-800 w-full mb-3"></div>}
            <nav className="space-y-1.5">
        ...
        {currentRole === 'vendor' && (
          <div>
            {!isCollapsed && <div className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-4 mb-3 mt-6">Advance Payments</div>}
            {isCollapsed && <div className="h-px bg-slate-800 w-full mb-3 mt-6"></div>}
            <nav className="space-y-1.5">
        ...
        {currentRole === 'vendor' && (
          <div>
            {!isCollapsed && <div className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-4 mb-3 mt-6">Revenue Share</div>}
            {isCollapsed && <div className="h-px bg-slate-800 w-full mb-3 mt-6"></div>}
            <nav className="space-y-1.5">
              <NavItem to="/vendor/payment" icon={Icons.money} label="Revenue Share" />
            </nav>
          </div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-slate-800/80">
        <div 
          ...
          onClick={isCollapsed ? handleLogout : () => navigate(currentRole === 'admin' ? '/admin/profile' : '/vendor/profile')} 
          title={isCollapsed ? "Logout" : "View Profile settings"}
        >
          <div className="flex bg-gradient-to-tr from-accent to-orange-400 text-white items-center justify-center w-10 h-10 rounded-lg flex-shrink-0 font-bold shadow-lg shadow-accent/20">
            {initials}
          </div>
          {!isCollapsed && (
            <div className="flex flex-col flex-1 truncate">
              <span className="text-sm font-bold text-white truncate">{displayName}</span>
              <span className="text-xs text-slate-400 font-medium">{currentRole === 'admin' ? 'Administrator' : 'Vendor'}</span>
            </div>
          )}
          {!isCollapsed && (
            <button 
              ...
              title="Logout"
              className="text-slate-500 hover:text-destructive p-2 rounded-lg hover:bg-slate-800 transition-colors focus:outline-none"
            >
